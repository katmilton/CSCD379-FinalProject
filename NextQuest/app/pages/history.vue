...
const mappedHistory = computed(() =>
  historyItems.value.map((item) => {
    let parsed: any = {}
    try { parsed = JSON.parse(item.resultJson) } catch {}
    return {
      id: item.id,
      prompt: item.prompt,
...
<template>
  <v-container class="page-section section-stack fade-in">
    <SectionTitle eyebrow="Your activity" title="Recommendation history" subtitle="Revisit your past prompts and remember what the AI suggested." />

    <div v-if="loading" class="text-medium-emphasis">Loading your history...</div>
    ...
    <v-card v-else class="glass-card pa-8 text-center" rounded="2xl">
      ...
      <div class="mb-4" style="color: rgba(255,255,255,.66)">Search for a game and your recommendation sessions will appear here.</div>
